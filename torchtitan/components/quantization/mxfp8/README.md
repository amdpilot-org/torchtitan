...
- **Dynamic activation quantization**: Linear and Grouped GEMM activations use
  ...
  backward, or retain a columnwise MXFP8 representation produced in forward.
...
- **FSDP-managed dense weights**: After FSDP all-gathers a dense Linear weight
  ...
  scale tiles to create FPROP and DGRAD representations. FSDP owns those
  ...
  released after the independent MXFP8 representation is constructed.
...
two independently quantized weight representations for the two GEMM
orientations.
...
- The generic FSDP compute-weight lifecycle and its MXFP8 specialization.
...
separate module-level or autograd-level weight cache. The compute weight
...
installs the compute-weight wrapper immediately before sharding. This reuses
...
MXFP8 representation would then increase peak memory. Since this ownership is
...
Without activation checkpointing, the selected representation remains live
...
With full activation checkpointing, neither representation is retained from
...
MXFP8-selected linear therefore produces rowwise and columnwise representations
