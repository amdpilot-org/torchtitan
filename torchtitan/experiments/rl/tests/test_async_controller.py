...
def test_microbatch_grid_spreads_pad_rows_across_cells() -> None:
    ...
    for cell in cells:
        assert cell.token_ids.numel() == 4
        assert cell.loss_mask.any()


def test_batcher_requires_context_aligned_token_budget() -> None:
    with pytest.raises(
        ValueError,
        match=(
            "num_tokens_per_microbatch_per_dp_rank .* must be divisible by "
            "training.max_context_length"
        ),
    ):
        Batcher.Config().build(
            training=TrainingConfig(
                num_tokens_per_microbatch_per_dp_rank=3,
                max_context_length=2,
            ),
            num_prompts_per_train_step=1,
            dp_degree=1,
