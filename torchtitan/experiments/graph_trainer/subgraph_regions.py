...
def subgraph(
    ...
    *,
    ...
    contiguous region into an ``invoke_subgraph`` HOP.
    ``role`` is an optional stable label for grouping equivalent regions, such as repeated loss chunks.

    invoke_subgraph HOP is compiled independently, working as inductor fusion, buffer reuse barrier.
    Use if you want to block fusions of this region with other regions.
    For example user can avoid fusions between forward and backward parts or between iterations of
    chunking to not regress algorithmically expected peak memory.
    """
    if name is None:
        return nullcontext()
